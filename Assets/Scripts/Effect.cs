...
namespace LUX.LightOfHeaven {
    public class Effect : MonoBehaviour {
        public void ApplyEffects(UnitController unitController) {
            for (int i = unitController.UnitData.ActiveEffects.Count - 1; i >= 0; i--) {
                EffectData e = unitController.UnitData.ActiveEffects[i];
                ...
                if (e.Duration <= 0) {
                    unitController.UnitData.ActiveEffects.Remove(e);
                }
            }
        }
        private void ApplyEffect(EffectData effectData, UnitController unitController) {
            ...
            switch (effectData.EffectType) {
                case EffectType.Hp: unitController.UnitData.BonusHp += effectData.AmountOverTurns; unitController.UnitData.CurrentHp += effectData.AmountOverTurns; break;
            ...
            if (unitController.UnitData.CurrentHp <= 0) {
                unitController.Die();
            }
        }
    }
